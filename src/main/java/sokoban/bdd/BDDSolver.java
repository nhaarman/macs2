...
public class BDDSolver implements Solver
{
   ...
   public boolean solve(final String initialLurd)
   {
     ...
     */
      ...
      while (!vOld.equals(vNew) && vNew.and(mGoalBDD).satCount(mVariableSet)
              == 0)
      {
         vOld = vNew;
         vNew = vOld.or(applyTransition(vOld, mTransitionBuilder
                 .allTransitions()));
   ...
   private String findTrace()
   {
      ...
      ListIterator<BDD> iterator = mIntermediateBDDs.listIterator
              (mIntermediateBDDs.size());

      BDD finalSetOfStates = iterator.previous(); //there is at least initial
      // state
      ...
      while (iterator.hasPrevious())
      {
         ...
         else if (cameFromThisSide(previous, stateToLookFor, Direction.DOWN))
         {
            ...
            stateToLookFor = applyTransition(stateToLookFor,
                    mTransitionBuilder.downTransitionReversed()).and(previous);
         }
         else
         {
      ...
      for (int i = 0; i < lurd.length(); i++)
      {
         ...
         switch (lurd.charAt(i))
         {
            ...
            case 'd':
               ...
               break;
            default:
               throw new IllegalArgumentException("Invalid character in lurd " +
                       "string: " + lurd.charAt(i));
         }
